...
data = pd.read_csv('RandomUniformD2.csv')
...
for selection in selections:
    ...
    for iteration in range(40):
        print(f"{iteration=}")
        model = gp_population(X_train, y_train, true_class, 1, generations_count=50, mutation_probability=0.7, selection_type=selection, depth=6)
        model.fit()
