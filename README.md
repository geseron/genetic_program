...
В качестве реальной задачи в работе использована проблема с репозитория UCI «Land Mines».
Выборка представлена тремя признаками и метками классов объектов.

Важно учесть, что в данной работе рассматривается задача бинарной классификации, поэтому метки класса «нет мины» приняли 0, а метки классов, обозначающие различные мины приняли 1.
В качестве признаков представлены:
- напряжение $V$
- высота датчика от земли $H$
- тип почвы $S$

На рисунке ниже покажем график распределения данных с метками классов.

<div align="center">

!['Распределение данных'](images/land_mines_distr.jpg)

</div>

Для построения решающего правила была выбрана переменная «Высота», а в качестве её аргументов «Напряжение» и «Тип почвы». График $H(V)$ представим ниже.

<div align="center">

!['График рассеянья H(v)'](images/land_m_dr.png)

</div>

В результате работы программы получены следующие результаты. График разбиения

<div align="center">

!['График рассеянья H(v)'](images/final_rule.png)

</div>

И получившаяся фомула функции разбиения:

$$
-1.76 + \frac{0.73}{\sin(V)}
$$

Как можно увидеть хоть алгоритму и была дана воможность воспользоваться всеми переменными, по завершению осталась только одна. Точность решения составила 0,85.

Конечно используя разбиение на классы только по напряжению, довольно трудно достичь 100% точности, но учитывая то, что это базовый алгоритм генетического программирования, можно говорить об успешном прохождении алгоритмов "проверки" на применимость к реальным задачам.

# Итоги

Осуществлено исследование влияния вида селекции на результативность метода. Три из четырёх опытов показали, что статистически значимой разни-цы с уровнем значимости 0,05 не наблюдается.

Была описана и решена задача по детектированию мин из публичного хранилища данных. Точность решения задачи составила 0,85.

Подводя итог можно сказать, что генетическое программирование явля-ется мощным инструментом для решения задачи бинарной классификации. Этот метод позволяет не только находить решение, но и представлять его в интерпретируемом виде.
